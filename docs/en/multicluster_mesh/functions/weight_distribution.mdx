...
Refer to the [Access Verification](../how_to/deploy_bookinfo.mdx#access-verification) method for the productpage service to continuously access the productpage service based on the routing configuration (`External Access Address`) created by the gateway deployed in cluster `c1`.
