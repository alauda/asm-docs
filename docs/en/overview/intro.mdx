...
The platform offers one-click infrastructure deployment and upgrade capabilities, visual service governance capabilities, efficient application performance management capabilities, and highly available and high-performance service gateway management capabilities. It provides a complete set of stable, reliable, and open microservice solutions for enterprises undergoing digital transformation. This helps enterprises improve service governance efficiency, significantly reduce the maintenance costs of microservice frameworks, and reduce developers' dependency on frameworks, allowing enterprises to focus on business development and continuously enhance their core competitiveness.

## Advantages
### Ease of Use

* One-click deployment and upgrade of service mesh via visual forms[6,14](@ref).
* Experience rich non-intrusive service governance capabilities ​**without development costs** by simply deploying the service mesh[6,14](@ref).

### Distributed Microservices System

Replaces traditional monolithic architectures with distributed microservices to meet modern business demands including rapid scaling, elastic resource allocation, agile iteration, and accelerated delivery[6,9](@ref).

### Cloud-Native Microservice Lifecycle Management

* Seamless integration with cloud-native ecosystems (container platforms, DevOps) to provide ​**full lifecycle management** - from development/testing to deployment/operation/governance - supporting enterprise containerization and microservice migration[6,9](@ref).
* Enterprise-grade multi-tenant system enables resource/personnel isolation across projects (e.g., subsidiaries/departments/teams), ensuring cross-project security and compliance[6,9](@ref).

### Simplified Canary Release

Automated (or controlled) canary releases between service versions based on dynamic traffic policies[6,12,14](@ref).

### Comprehensive & Reliable Service Governance

Platform offers visual governance strategies for:
- Dynamic routing configuration
- Circuit breaking
- Rate limiting
- Instance-level isolation  
- Load balancing rules
Covering ​**90%+** of microservice governance scenarios[6,9,23](@ref).

### Global Service Observability

Built on ​**OpenTelemetry** and ​**Istio**, the platform provides:
1. ​**360° Traffic Monitoring**: Real-time metrics for API performance, latency, and error rates[18,23](@ref).
2. ​**Distributed Tracing**: Full-link tracking to pinpoint bottlenecks and dependencies[18](@ref).
3. ​**Multi-Dimensional Analysis**: Drill-down capabilities for optimization and troubleshooting[18](@ref).

### Non-Intrusive Data Plane Extensions

Extend traffic management via ​**WebAssembly (WASM)** plugins:
- Manage WASM artifacts/configs as OCI images[6](@ref)
- Deploy extensions via ​**ECDS (Extension Config Discovery Service)** API with centralized supervision[6](@ref).

### Multi-Region Disaster Recovery

High-availability architecture with:
- ​**Independent Clusters**: Fault isolation ensures single cluster failures don't impact others[6,24](@ref)
- ​**Automatic Traffic Redirection**: Failed services trigger traffic shifts to healthy clusters[6,24](@ref)
- ​**Geo-Specific Priority**: Customize failover priorities based on cluster locations[6,24](@ref).

### Flexible Multi-Gateway Management

- ​**Zero-Downtime Deployment**: Manage gateways across ports with monitoring/alerting[6,28](@ref)
- ​**API Publishing**: Visual control for external service/API grouping and routing[6,28](@ref).

### End-to-End Ops System

* ​**Integrated Monitoring**: Combines service/API traffic metrics, JVM monitoring, and alerts for rapid issue resolution[6,18](@ref).
* ​**Unified Logging**: End-to-end log collection, analysis, and search capabilities to accelerate troubleshooting[6,18](@ref).
