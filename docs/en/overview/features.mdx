...
| **Service Governance** | - Provides different governance capabilities for different types of services (services in Service Mesh architecture, Java services). You can choose the appropriate governance method based on the service type.<br/>- Configure service governance strategies such as load balancing, connection pool management, circuit breaking, and service security for services using visual forms. |
| **Service Routing** | - Manage the routing of all traffic accessing services.<br/>- Supports traffic routing management based on multiple sets of conditional rules, with options for uri, scheme, method, headers, port, sourceLabels, and various matching methods. |
...
## Reliability Governance of Service Connections

Services within the mesh interact remotely using protocols such as HTTP/HTTP2 and gRPC. When a service response is slow or unavailable, it can cause a "cascading failure" effect, occupying more resources from preceding services and potentially leading to a system crash.

To avoid this, protective mechanisms and fault simulations must be established for service calls to ensure the reliability of service connections and enhance the overall stability of the microservice system.

| Feature | Description |
| --- | --- |
| **Traffic Strategies** | The platform provides a unified service management perspective, allowing users to configure governance strategies such as load balancing, connection pool management, and service circuit breaking through visual forms.<br/>- **Load Balancing Strategy**: Distributes traffic among service pods automatically based on the specified load balancing strategy.<br/>- **Circuit Breaking Strategy**: Detects abnormal service instances, handles exceptions seamlessly, isolates faulty instances automatically, and reduces the impact on downstream services.<br/>- **Connection Pool Settings Strategy**: Rejects some requests by configuring appropriate parameters to prevent system crashes caused by excessive traffic. |
| **Routing Rules and Strategies** | Supports dynamic configuration of fault recovery and fault injection strategies, including error injection, delay return, timeout retry, request rewriting, and traffic mirroring during service runtime. These strategies assist in ensuring the reliable operation of services and prevent local faults from cascading to other services. |
...
| **Blacklist and Whitelist** | By setting blacklists or whitelists for services, you can control access permissions between services within the same mesh. |
