...
- Multi-dimensional Querying: Supports 9 combinations of query conditions such as TraceID, service name, labels, etc.
...
<Steps>
  ...
  **Tip**: Query conditions can be combined for use. You can refine your query by adding multiple query conditions.
  ...
  | **Span kind**      | **Root Span**: Searches for root Spans initiated by the configured **service**. This search mode is used when the configured service is the initiator of the entire call request. <br />**Service Entry Span**: Searches for the first Span generated when the configured **service** is called as a server.         |
  ...
  - Once you select the query conditions and enter the respective values, click the **Add to query** button, and the current conditions will be displayed in the **Query** result area, triggering the query.
  - You can also expand **Common search condition** to quickly add recently used search conditions.
</Steps>
