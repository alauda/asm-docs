...
<Steps>
  ...
  **Tip**: Query conditions can be combined for use. You can refine your query by adding multiple query conditions.
  ...
  | **API**           | The API added for the service in **Traffic Monitoring > API Management**.       |
  | **Tag**           | You can filter the query results by inputting tags (Tag). Supported tags include tags found in the Span details. |
